<div class="category_product_page">
          ...
          <div class="single_product_list product_tricker">
            <div class="product_img">
              ...
              <div class="socal_icon">
                <a href="" class="add_to_wishlist {{$product->is_wishlist() == 1?'is_wishlist':''}}" id="coupon_category_all_product_wishlistbtn_{{$product->id}}" data-product_id="{{$product->id}}" data-seller_id="{{$product->user_id}}"> <i class="ti-heart"></i> </a>
  ...
  @php
      $products = $allCouponCategoryProducts;
      ...
      <div class="pagination_part">
        <div class="mt-10"></div>
          <nav aria-label="Page navigation example">
              <ul class="pagination">
                  <li class="page-item all_coupon_category_page_item"><a class="page-link" href="{{ $products->previousPageUrl() }}"> <i class="ti-arrow-left"></i> </a></li>
                  @for ($i=1; $i <= $total_number_of_pages; $i++)
                      @if (($products->currentPage() + 2) == $i)
                          <li class="page-item all_coupon_category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if (($products->currentPage() + 1) == $i)
                          <li class="page-item all_coupon_category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if ($products->currentPage() == $i)
                          <li class="page-item all_coupon_category_page_item @if (request()->toRecievedList == $i || request()->toRecievedList == null) active @endif"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if (($products->currentPage() - 1) == $i)
                          <li class="page-item all_coupon_category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      ...
                      @endif
                  @endfor
                  <li class="page-item all_coupon_category_page_item"><a class="page-link" href="{{ $products->nextPageUrl() }}"> <i class="ti-arrow-right"></i> </a></li>
              </ul>
          </nav>
          <br>
      </div>
