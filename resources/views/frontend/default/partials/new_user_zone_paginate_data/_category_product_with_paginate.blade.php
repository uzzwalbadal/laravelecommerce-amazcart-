...
<div class="category_product_page">
  <div class="row">
      @foreach(@$category->category->sellerProductWithPaginate() as $key => $product)

      <div class="col-lg-4 col-xl-3 col-sm-6 col-md-6 single_product_item">
          <div class="single_product_list product_tricker">
            <div class="product_img">
              <a target="_blank" href="{{singleProductURL(@$product->seller->slug, $product->slug)}}" class="product_img_iner">
                <img src="{{asset(asset_path(@$product->product->thumbnail_image_source))}}" alt="#" class="img-fluid">
              </a>
              <div class="socal_icon">
                <a href="" class="add_to_wishlist {{$product->is_wishlist() == 1?'is_wishlist':''}}" id="category_product_wishlistbtn_{{$product->id}}" data-product_id="{{$product->id}}" data-seller_id="{{$product->user_id}}"> <i class="ti-heart"></i> </a>
            ...
            <div class="product_text">
              <h5>
                <a target="_blank" href="{{singleProductURL(@$product->seller->slug, $product->slug)}}">@if(@$product->product_name) {{substr(@$product->product_name,0,28)}} @if(strlen(@$product->product_name) > 28)... @endif @else {{substr(@$product->product->product_name,0,28)}} @if(strlen(@$product->product->product_name) > 28)... @endif @endif</a>
              </h5>
  ...
  @php
      $products = @$category->category->sellerProductWithPaginate();
  ...
  <div class="col-lg-12">
      ...
      <div class="pagination_part">
          ...
          <nav aria-label="Page navigation example">
              <ul class="pagination" id="{{$category->id}}">
                  <li class="page-item category_page_item"><a class="page-link" href="{{ $products->previousPageUrl() }}"> <i class="ti-arrow-left"></i> </a></li>
                  @for ($i=1; $i <= $total_number_of_pages; $i++)
                      @if (($products->currentPage() + 2) == $i)
                          <li class="page-item category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if (($products->currentPage() + 1) == $i)
                          <li class="page-item category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if ($products->currentPage() == $i)
                          <li class="page-item category_page_item @if (request()->toRecievedList == $i || request()->toRecievedList == null) active @endif"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      @if (($products->currentPage() - 1) == $i)
                          <li class="page-item category_page_item"><a class="page-link" href="{{ $products->url($i) }}">{{ $i }}</a></li>
                      @endif
                      ...
                      @endif
                  @endfor
                  <li class="page-item category_page_item"><a class="page-link" href="{{ $products->nextPageUrl() }}"> <i class="ti-arrow-right"></i> </a></li>
              </ul>
          </nav>
          <br>
      </div>
