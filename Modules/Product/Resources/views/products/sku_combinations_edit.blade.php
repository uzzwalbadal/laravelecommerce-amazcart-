...
@foreach ($combinations as $key => $combination)
	...
	@if(strlen($str) > 0)
			<tr class="variant text-center">
				...
				<td class="pt-25">
					<input  class="primary_input_field mt-30" type="number" name="selling_price_sku[]" value="{{ ($query_1) ? @$query_1->selling_price : "0" }}" min="0" step="{{step_decimal()}}" class="form-control" required>
				</td>
				@if(!isModuleActive('MultiVendor'))
				<td class="text-center pt-25 stock_td d-none">
					<input class="primary_input_field mt-30" type="number" name="sku_stock[]" value="{{ ($query_1) ? @$query_1->product_stock : "0" }}" min="0" step="0" class="form-control" required>
				</td>
				@endif
				<td class="pt-25">
					<input class="primary_input_field mt-30" type="text" name="sku[]" value="{{ ($query_1) ? @$query_1->sku : $sku }}" class="form-control">
				...
				<td class="variant_physical_div text-center pt_2">
					<div class="primary_input mb-10">
						<div class="sku_img_div">
							<img id="variantImgDiv_{{$valIncre}}" class="variantImgDiv" src="{{showImage((@$query_1->variant_image) ? @$query_1->variant_image:'backend/img/default.png')}}" alt="">
						</div>
					...
					<div class="primary_input">
						...
						<div class="primary_file_uploader">
						  ...
						  <button class="mt-30" type="button">
							  ...
							  <input type="file" class="d-none variant_digital_file_change" name="digital_file[]" id="digital_file_{{$valIncre}}" data-name_id="variant_digital_file_{{$valIncre}}">
						  </button>
					   </div>
					</div>
				</td>
